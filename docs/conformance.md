...
| XML edit references | Correctly namespaced `val:EditRef` loads through the real parser. The reusable `editref-staterule.xml` fixture exposed and now guards the constructor mapping. Support is scoped to the generic `Edit` contexts (e.g. `StateRule`); an `EditRef` under a strategy- or strategies-level `Edit` is rejected at load with `InconsistentStrategyException` rather than silently discarded, guarded by `ParserDomainExceptionTests`. |
