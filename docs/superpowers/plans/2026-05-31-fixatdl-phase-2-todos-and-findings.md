...
**Tech Stack:** .NET 10; xUnit v3 + AwesomeAssertions (`.Should()`) + NSubstitute (test project global usings: `AwesomeAssertions`, `NSubstitute`, `Xunit`); `dotnet build` 0-warning bar; `dotnet test`; CI coverage floor (65%) in `build-and-test.yml`. *(Correction, 2026-08-23: that floor was removed with `build-and-test.yml`; no coverage floor is currently enforced in CI — see `docs/coverage-baseline.md`.)*
